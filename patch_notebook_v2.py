...
# 1. Update architecture for Dense bottleneck and MAE
for c in nb['cells']:
    if c.get('cell_type') == 'code':
        source = "".join(c.get('source', []))
        
        if 'def build_conv_autoencoder' in source:
            new_source = """
...
"""
        ...
        # Update plotting labels from MSE to MAE
        if 'plt.ylabel("MSE")' in source:
            c['source'] = [line.replace('MSE', 'MAE').replace('mse', 'mae') for line in c['source']]
            
        # Update reconstruction_mse to reconstruction_mae
        if 'def reconstruction_mse' in source:
            new_source = """
...
"""
        ...
        # Update plotting for MAE
        if 'Reconstruction MSE Distribution' in source:
            c['source'] = [line.replace('mse', 'mae').replace('MSE', 'MAE') for line in c['source']]
